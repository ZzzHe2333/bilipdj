...
console.log("[PDJ] config summary", {
    admin_count: admins.filter(Boolean).length,
    ban_count: ban_admins.filter(Boolean).length,
    jianzhang_count: jianzhang.filter(Boolean).length
});
//预写入-
...
var pdjArchiveLoaded = false;
var pdjHidePrivateLogs = true;

function PDJ_PrivateText(value) {
    var text = String(value || "").trim();
    if (!text) {
        return "-";
    }
    return pdjHidePrivateLogs ? "[hidden]" : text;
}

function PDJ_ListCount(value) {
    return Array.isArray(value) ? value.filter(Boolean).length : 0;
}

function PDJ_LogQueueSummary(label) {
    console.log(label, { queue_size: PDJ_ListCount(persons) });
}

function PDJ_LogIncomingSummary(data) {
    if (!data || typeof data !== "object") {
        return;
    }

    var summary = {
        cmd: typeof data.cmd === "string" ? data.cmd : (data.type || "unknown")
    };

    if (summary.cmd === "PDJ_STATUS" || data.type === "PDJ_STATUS") {
        summary.status = data.status || "unknown";
        summary.transport = data.transport || "";
        summary.host = data.host || "";
        summary.port = Number(data.port || 0);
        console.log("[PDJ] 事件摘要", summary);
        return;
    }

    if (data.result && typeof data.result === "object") {
        summary.user = PDJ_PrivateText(data.result.uname || "");
        summary.msg_chars = String(data.result.msg || "").length;
    }
...
async function PDJ_LoadConfig() {
    try {
        ...
        PDJ_EmitStatus('config_loaded', {
            admin_count: PDJ_ListCount((cfg.myjs || {}).admins),
            ban_count: PDJ_ListCount((cfg.myjs || {}).ban_admins),
            jianzhang_count: PDJ_ListCount((cfg.myjs || {}).jianzhang)
        });
    } catch (err) {
...
function applyMyJSConfig(mycfg) {
    ...
    console.log("[PDJ] config override", {
        admin_count: admins.filter(Boolean).length,
        ban_count: ban_admins.filter(Boolean).length,
        jianzhang_count: jianzhang.filter(Boolean).length,
        paidui_list_length_max: paidui_list_length_max
    });
}
...
async function PDJ_Connect() {
    ...
    ws.onmessage = async function (msgEvent) {
        ...
        if (maybeJson && maybeJson.type === 'PDJ_STATUS') {
            PDJ_LogIncomingSummary(maybeJson);
...
function PDJ_Send(payload) {
    if (!ws || ws.readyState !== WebSocket.OPEN) {
        console.warn('[PDJ] WS未连接，消息已丢弃', {
            payload_chars: String(payload || '').length
        });
        return;
    }
    ws.send(payload);
...
function PDJ_RestoreQueueMarkup(queueItem) {
    var item = String(queueItem || "").trim();
    if (!item) {
        return "";
    }
    ...
    if (item.indexOf("<span") >= 0) {
        return item
            .replace(/\sclass="pdj-confirm-pending"/g, "")
            .replace(/(<span[^>]*>)(\s*(?:⏳待确认|等待确认)\s*)+/g, "$1");
    }

    var plain = item.replace(/^(\s*(?:⏳待确认|等待确认)\s*)+/g, "").trim();
    if (!plain) {
        return "";
    }
    return "<span>" + escapeHtml(plain) + "<br></span>";
}

async function PDJ_LoadQueueArchive() {
    if (pdjArchiveLoaded) {
        return;
    }
    pdjArchiveLoaded = true;

    try {
        var res = await fetch('/api/queue/archive');
        if (!res.ok) {
        ...
        if (!payload || !payload.enabled) {
            ...
            console.log("[PDJ] 队列存档已关闭，跳过恢复");
            return;
        }
        var queueItems = Array.isArray(payload.queue) ? payload.queue : [];
        clearPendingConfirmTask();
        persons = queueItems.map(PDJ_RestoreQueueMarkup).filter(Boolean);
        document.getElementById('danmu').innerHTML = persons.join("");
        console.log("[PDJ] 队列存档恢复", {
            slot: Number(payload.slot || 0),
            queue_size: queueItems.length,
            timestamp: payload.timestamp || ""
        });
    } catch (err) {
        console.warn('[PDJ] 队列存档读取失败', err);
    }
}
...
function startDelFirstPendingConfirm() {
    ...
    pendingConfirmTask = {
        ...
        timerId: setTimeout(function () {
            ...
            console.log("[叫号确认]超时自动跳过:", PDJ_PrivateText(timeoutPerson));
    ...
    console.log("[叫号确认]等待确认:", PDJ_PrivateText(pendingPerson), "超时秒数:", confirm_wait_seconds);
}
...
function ws_zbtool_open(){
    ...
    ws_zbtool.onmessage = async function (msgEvent) {
        console.log("[PDJ] zbtool event", { payload_chars: String(msgEvent.data || '').length })
...
function sendMessage(xz_orz){ 
    if (fankui == true){
        console.log("排队人员：", PDJ_PrivateText(xz_orz));
...
function userList_in_String(userList = [],UserStr = ""){
    ...
    for (let i = 0; i < userList.length; i++) {
        ...
        if (UiS_index !== -1) {
          console.log("[PDJ] user list matched", { index: UiS_index });
    ...
    //.indexOf是查找字符的位置
                ...
                if(add_op_Idex >= 0){
                    ...
                    if(add_op_message != ""){
                        ...
                        console.log("[本场管理员数量]:", admins.length)
                    };
                };
                //取消管理员 xxx
...
function consolelogprint(kind = 1,person = "",message = ""){
    if (kind == 1){
        console.log("[接收信息]", {
            person: PDJ_PrivateText(person),
            message: PDJ_PrivateText(message),
            queue_size: persons.length
        });
        PDJ_LogQueueSummary("[排队列表]");
        console.log("-------------------------------------",);}
    if(kind == 2){
        console.log("[本场黑名单数量]:", ban_admins.length)
    }
    if(kind == 3){
        console.log("[本场管理员数量]:", admins.length)
    }
        ...
        case 'danmu': //case 'DANMU_MSG':
            ...
            if (pendingConfirmTask && pendingConfirmTask.person == person) {
                if (confirmWords.indexOf(normalizedMsg) !== -1) {
                    ...
                    console.log("[叫号确认]确认成功:", PDJ_PrivateText(person));
                    document.getElementById('danmu').innerHTML=persons.join("");
                    return;
                }
            }
            ...
            if(ban_user_inIdex != -1){
                console.log("[排队权限]已禁止黑名单排队||排队无效--->", PDJ_PrivateText(person));
                return
            };
            ...
            if(all_suoyourenbukepaidui == true && op_user_inIdex == -1){
                console.log("[排队权限]已禁止所有人排队||排队无效--->", PDJ_PrivateText(person));
                return
            };
            //查看是不是已有舰长--->查看是不是B站舰长--->尝试添加为插件舰长--->更新舰长值
            if (jianZ_inIdex == -1 && is_jianzhang == 1){
                    console.log("[添加权限]舰长:", PDJ_PrivateText(person));
                    console.log("[权限列表数量]:", jianzhang.length);
            ...
            if(op_user_inIdex == -1){        
                if(user_kind_room == 2){
                    console.log("[添加权限]主播:", PDJ_PrivateText(person)); 
                    console.log("[权限列表数量]:", admins.length);
                    admins.push(person);
                    op_user_inIdex = admins.indexOf(person);//更新用户管理员
                };                  
                if(user_kind_room == 1 && fangguan_can_doing == true){                    
                        console.log("[添加权限]房管:", PDJ_PrivateText(person));
                        console.log("[权限列表数量]:", admins.length);
                        admins.push(person)
                        op_user_inIdex = admins.indexOf(person);//更新用户管理员
            ...
            if(paidui_list_length >= paidui_list_length_max && index == -1 ){
                ...
                if(op_user_inIdex < 0){
                    console.log("[排队限制]到达排队上限||排队无效--->", PDJ_PrivateText(person));
                    return;
                };
                if(op_user_inIdex >= 0){
                    console.log("[排队限制]到达排队上限||管理员操作有效--->", PDJ_PrivateText(person));
                }
