...
HTTP_ENDPOINTS = [
    ...
    ep("GET", "/api/plugins/config?id=example.kuaishou.danmu", "读取插件动态配置", response='{"status":"ok","id":"example.kuaishou.danmu","configurable":true,"schema":{...},"values":{...},"secret_fields":{"access_token":true}}', notes="仅本机。secret 字段只返回是否已设置，不返回明文。"),
    ep("POST", "/api/plugins/config", "保存插件动态配置并重连", body='{"id":"example.kuaishou.danmu","values":{"room_id":"12345"},"unset":[]}', response='{"status":"ok","id":"example.kuaishou.danmu","configurable":true,...}', notes="仅本机。服务端按 manifest.config_schema 校验类型、required、enum 和范围；保存成功后重连对应弹幕 Relay。"),
    ep("POST", "/api/plugins/install", "安装本地 .bilipdj-plugin", body='{"filename":"kuaishou.bilipdj-plugin","data_base64":"...","allow_unsigned":false}', response='{"status":"ok","plugin":{...}}', notes="仅本机。插件包最大 1 MiB；安装阶段不执行插件代码。未签名包必须显式 allow_unsigned=true。语言类插件是 resource-only 资源包，不执行 Python/JavaScript。"),
    ep("POST", "/api/plugins/enable", "启用外部插件", body='{"id":"example.kuaishou.danmu"}', response='{"status":"ok","plugin":{...}}', notes="弹幕代码插件启用时才加载运行时；语言类插件启用后只注册翻译资源并进入语言选择列表。"),
    ep("POST", "/api/plugins/disable", "禁用外部插件", body='{"id":"example.kuaishou.danmu"}', response='{"status":"ok","plugin":{...}}'),
    ep("POST", "/api/plugins/uninstall", "卸载外部插件", body='{"id":"example.kuaishou.danmu"}', response='{"status":"ok","id":"example.kuaishou.danmu"}', notes="插件代码目录与 plugin config 条目会删除；独立 data 目录仍保留。"),
    ...
    ep("GET", "/api/language", "读取界面语言和已启用语言插件", response='{"status":"ok","default":"zh-CN","active":"en-US","languages":[...],"translations":{"设置":"Settings"}}', local=False, notes="翻译表以当前中文界面文字作为 key；缺失 key 由客户端自动回退原中文。"),
    ep("POST", "/api/language", "切换界面语言", body='{"language":"en-US"}', response='{"status":"ok","active":"en-US","languages":[...],"translations":{...}}', notes="仅可选择已启用且校验通过的 language 插件；zh-CN 为内置默认语言。"),
...
本目录由 `python .github/ci/generate_api_docs.py` 自动生成，并被仓库根 `.gitignore` 的 `/api/` 规则排除。
