...
from apps.windows import update_network
from apps.windows.updater_model import PIECE_COLUMNS, progress_for_message, select_update_log_root
...
class CategorizedLogTests(unittest.TestCase):
    def test_daily_names_use_kind_date_and_room(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(common.name, "common_20260730_125432.log"); self.assertEqual(error.name, "error_20260730_unknow.log"); self.assertEqual(update.name, "update_20260730_3049445.log")

    def test_room_token_uses_active_platform(self) -> None:
        self.assertEqual(log_manager.room_token_from_config({"platform": "bilibili", "bilibili": {"roomid": 123}}), "123")
        self.assertEqual(log_manager.room_token_from_config({"platform": "douyin", "douyin": {"live_id": "abc-456"}}), "abc-456")
        self.assertEqual(log_manager.room_token_from_config({}), "unknow")

    def test_cleanup_removes_only_expired_managed_logs(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir); directory = root / "log"; directory.mkdir()
            old = directory / "common_20260101_1.log"; fresh = directory / "error_20260730_1.log"; unrelated = directory / "notes.log"
            for path in (old, fresh, unrelated): path.write_text("x", encoding="utf-8")
            old_time = time.time() - 40 * 86400; os.utime(old, (old_time, old_time))
            self.assertEqual(log_manager.cleanup_logs(root, 31), 1); self.assertFalse(old.exists()); self.assertTrue(fresh.exists()); self.assertTrue(unrelated.exists())


class TypographyTests(unittest.TestCase):
    def test_style_guard_preserves_options_and_generates_css(self) -> None:
        module = types.SimpleNamespace(); module.DEFAULT_STYLE = {"queue_font_size": 50}; module.DEFAULT_CONFIG = {"style": {}}; module.state = {"queue_font_size": 50}; module.load_style = lambda: dict(module.state)
        def save_style(payload): module.state = dict(payload)
        def build_index_css(_payload=None): return ":root {\n    --queue-font-size: 50px;\n}\n"
        module.save_style = save_style; module.build_index_css = build_index_css
        self.assertTrue(patch_style_module(module)); module.save_style({"queue_letter_spacing": 4}); self.assertEqual(module.state["queue_letter_spacing"], 4)
        for key in STYLE_OPTION_DEFAULTS: self.assertIn(key, module.state)
        css = module.build_index_css(module.state); self.assertIn("--queue-letter-spacing: 4px", css); self.assertIn("--queue-line-height:", css); self.assertIn(".queue-content", css)

    def test_bundled_css_exposes_advanced_variables(self) -> None:
        source = (ROOT / "apps" / "web" / "static" / "moren.css").read_text(encoding="utf-8")
        for variable in ("--queue-font-family", "--queue-letter-spacing", "--queue-word-spacing", "--queue-line-height", "--queue-item-gap", "--queue-text-align", "--queue-text-opacity"):
            self.assertIn(variable, source)


class UpdaterAnimationTests(unittest.TestCase):
    def test_tetris_sequence_is_fixed_and_fills_two_rows(self) -> None:
        self.assertEqual(PIECE_COLUMNS, (4, 0, 8, 2, 6)); occupied = set()
        for column in PIECE_COLUMNS: occupied.update((column + dx, 8 + dy) for dx, dy in ((0, 0), (1, 0), (0, 1), (1, 1)))
    ...
    def test_update_progress_follows_fixed_stages(self) -> None:
        self.assertEqual(progress_for_message("解压更新包", 3), 22); self.assertEqual(progress_for_message("备份当前版本", 22), 46); self.assertEqual(progress_for_message("启动成功", 90), 100)

    def test_log_root_follows_directory_rename(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir); app = root / "app"; backup = root / ".app.update-backup"; app.mkdir(); self.assertEqual(select_update_log_root(app), app); app.rename(backup); self.assertEqual(select_update_log_root(app), backup)
...
class SourceWiringTests(unittest.TestCase):
    ...
    def test_style_save_is_background_and_overlay_restart_is_suppressed(self) -> None:
        source = (ROOT / "apps" / "windows" / "control_panel_features.py").read_text(encoding="utf-8")
        self.assertIn('name="bilipdj-style-save"', source); self.assertIn("_bilipdj_suppress_overlay_restart", source); self.assertIn("_bilipdj_force_sync_style_save", source)
