...
from apps.windows.control_panel_guard import patch_control_panel_class
from apps.windows.updater import UpdaterError
from apps.windows.updater_v2 import replace_path_with_retry
...
class RequestedUiLayoutTests(unittest.TestCase):
    def test_login_page_no_longer_contains_display_switches(self) -> None:
        source = (ROOT / "apps" / "web" / "static" / "config.html").read_text(encoding="utf-8")
    ...
    def test_queue_page_contains_only_queue_container(self) -> None:
        source = (ROOT / "apps" / "web" / "static" / "index.html").read_text(encoding="utf-8")
    ...
    def test_overlay_controls_are_defined_in_desktop_guard(self) -> None:
        source = (ROOT / "apps" / "windows" / "control_panel_guard.py").read_text(encoding="utf-8")
...
class ThemePersistenceTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        class ControlPanelApp:
            def _build_ui(self): return None
            def _build_overlay_tab(self, _frame): return None
            def _toggle_theme(self): self._dark_mode = not self._dark_mode
            def load_from_file(self): return None
            def gather_config(self): return {"ui": {"language": "中文"}}
    ...
    def test_theme_is_included_in_normal_config_save(self) -> None:
        panel = self.ControlPanelApp(); panel._dark_mode = False
        self.assertEqual(panel.gather_config()["ui"]["theme"], "light")
    ...
    def test_toggle_persists_theme_immediately(self) -> None:
        panel = self.ControlPanelApp(); panel._dark_mode = False
        panel._toggle_theme(); self.assertEqual(self.module.backend.config["ui"]["theme"], "dark")
        panel._toggle_theme(); self.assertEqual(self.module.backend.config["ui"]["theme"], "light")


class UpdaterDirectoryRetryTests(unittest.TestCase):
    def test_directory_move_succeeds(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir); source = root / "current"; destination = root / "backup"
            source.mkdir(); (source / "main.exe").write_bytes(b"test")
            replace_path_with_retry(source, destination, attempts=2, delay=0.01)
            self.assertFalse(source.exists()); self.assertTrue((destination / "main.exe").is_file())
    ...
    def test_persistent_access_denied_has_clear_error(self) -> None:
        with mock.patch.object(Path, "replace", side_effect=PermissionError(13, "denied")):
            with self.assertRaisesRegex(UpdaterError, "后端、透明窗口和杀毒软件"):
                replace_path_with_retry(Path("current"), Path("backup"), attempts=2, delay=0.01)

    def test_updater_spec_uses_migrated_entry(self) -> None:
        source = (ROOT / "apps" / "windows" / "updater.spec").read_text(encoding="utf-8")
        self.assertIn('"apps" / "windows" / "updater_gui.py"', source)
        self.assertIn('"apps.windows.updater_v2"', source)
        self.assertIn('"apps.server.log_manager"', source)
