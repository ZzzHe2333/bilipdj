...
class ProviderCatalogTests(unittest.TestCase):
    def test_required_presets_and_corrected_minimax_endpoint(self) -> None:
        catalog = get_provider_catalog()
        required = {"openai", "deepseek", "minimax", "qwen", "xiaomi_mimo", "claude", "opencode", "custom"}
        self.assertTrue(required.issubset(catalog))
        self.assertEqual(catalog["minimax"]["base_url"], "https://api.minimax.io/v1")
        self.assertIn("MiniMax-M3", [m["id"] for m in catalog["minimax"]["models"]])
    ...
    def test_opencode_protocol_is_selected_per_model(self) -> None:
        responses = resolve_provider_config(provider="opencode", api_key="k", model="gpt-5.6-terra")
        chat = resolve_provider_config(provider="opencode", api_key="k", model="deepseek-v4-flash")
...
class OpenAIAnswerInterfaceTests(unittest.TestCase):
    ...
    def test_custom_responses_does_not_send_unsupported_store_by_default(self) -> None:
    ...
    def test_minimax_uses_max_completion_tokens(self) -> None:
        ...
        self.assertEqual(fake.chat.completions.calls[0]["max_completion_tokens"], 123)
    ...
    def test_opencode_model_override_switches_protocol(self) -> None:
        ...
        fake = _FakeClient(chat_response=response)
        client = OpenAIAnswerClient(provider="opencode", model="gpt-5.6-terra", client=fake)
    ...
    def test_empty_response_and_request_error(self) -> None:
        empty = SimpleNamespace(output_text="", output=[], id="empty", model="gpt-5.6")
        with self.assertRaises(OpenAIEmptyResponseError):
            OpenAIAnswerClient(provider="openai", client=_FakeClient(responses_response=empty)).ask("q")

        error = RuntimeError("network")
        error.request_id = "req-failed"
        with self.assertRaises(OpenAIRequestError) as ctx:
            OpenAIAnswerClient(provider="deepseek", client=_FakeClient(error=error)).ask("q")
    ...
    def test_convenience_function_builds_provider_client(self) -> None:
        ...
        self.assertEqual(created[0].options["base_url"], "https://dashscope.aliyuncs.com/compatible-mode/v1")
