...
def remove_path_with_retry(path: Path, *, attempts: int = 20, delay: float = 0.25) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as archive:
        for member in archive.infolist():
            ...
            try:
                ...
                raise UpdaterError(f"更新包包含不安全路径：{member.filename}") from exc
        archive.extractall(destination)


def validate_staging(staging_dir: Path, main_exe_name: str) -> None:
    if not (staging_dir / main_exe_name).is_file():
        raise UpdaterError(f"更新包中缺少主程序：{main_exe_name}")
...
def launch_main(app_dir: Path, main_exe_name: str) -> subprocess.Popen[bytes]:
    main_path = app_dir / main_exe_name
...
def perform_update(
    *,
    ...
    validate_staging(staging_dir, main_exe_name)
    ...
    try:
        ...
        process = launch_main(app_dir, main_exe_name)
        deadline = time.monotonic() + STARTUP_GRACE_SECONDS
        while time.monotonic() < deadline:
            return_code = process.poll()
            if return_code is not None:
                raise UpdaterError(f"新版主程序启动后提前退出，退出码 {return_code}")
            time.sleep(0.25)
        _write_log(log_path, f"v{target_version} 启动成功，保留上一版本备份：{backup_dir}")
    except Exception:
        rollback(app_dir, backup_dir, log_path)
        try:
            launch_main(app_dir, main_exe_name)
        except Exception as restart_error:
            _write_log(log_path, f"回滚后重新启动旧版本失败：{restart_error}")
        raise
    finally:
        remove_path_with_retry(staging_dir)
