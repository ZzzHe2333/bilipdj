"""Purple-mouse runtime integration for the existing multi-platform backend."""
...
def _install_relay_factory(server_module: Any) -> None:
    ...
    if not callable(original) or bool(getattr(original, "_bilipdj_purple_mouse_wrapped", False)):
        return

    def create_danmu_relay_with_purple_mouse(server: Any) -> Any:
        try:
            platform = str(
        ...
        if platform == "twitch":
            return twitch_protocol.TwitchDanmuRelay(server)
        return original(server)

    create_danmu_relay_with_purple_mouse._bilipdj_purple_mouse_wrapped = True  # type: ignore[attr-defined]
    server_module._create_danmu_relay = create_danmu_relay_with_purple_mouse
...
def _patch_issue79(issue79_module: Any) -> None:
    ...
    if callable(original_runtime) and not bool(getattr(original_runtime, "_bilipdj_purple_mouse_wrapped", False)):
        def runtime_for_platform_with_purple_mouse(server: Any, platform: str) -> dict[str, Any]:
            runtime = original_runtime(server, platform)
            if str(platform).strip().lower() == "twitch":
                section = dict(runtime.get("twitch", {}) or {})
                section["enabled"] = True
                runtime["twitch"] = section
            return runtime

        runtime_for_platform_with_purple_mouse._bilipdj_purple_mouse_wrapped = True  # type: ignore[attr-defined]
        issue79_module._runtime_for_platform = runtime_for_platform_with_purple_mouse

    original_writer = getattr(issue79_module, "_write_control_html", None)
    if callable(original_writer) and not bool(getattr(original_writer, "_bilipdj_purple_mouse_wrapped", False)):
        def write_control_html_with_purple_mouse(self: Any, server_module: Any) -> None:
            ...
            purple_js_tag = '<script src="/control_purple_mouse.js"></script>'
            ...
            if (Path(server_module.UI_DIR) / "control_redtv.js").is_file() and redtv_js_tag not in text:
                text = text.replace("</body>", f"  {redtv_js_tag}\n</body>")
            if purple_js_tag not in text:
                text = text.replace("</body>", f"  {purple_js_tag}\n</body>")
        ...
        write_control_html_with_purple_mouse._bilipdj_purple_mouse_wrapped = True  # type: ignore[attr-defined]
        issue79_module._write_control_html = write_control_html_with_purple_mouse


def install_twitch_runtime_guard(server_module: Any, issue79_module: Any | None = None) -> bool:
    if server_module is None:
        return False

    with _PATCH_LOCK:
        if bool(getattr(server_module, "_purple_mouse_runtime_guard_installed", False)):
            return True
        ...
        if isinstance(defaults, dict) and "twitch" not in defaults:
            defaults["twitch"] = copy.deepcopy(section_default if isinstance(section_default, dict) else {})

        _install_relay_factory(server_module)
        _patch_issue79(issue79_module)

        server_module.twitch_protocol = twitch_protocol
        server_module.TwitchDanmuRelay = twitch_protocol.TwitchDanmuRelay
        server_module._purple_mouse_runtime_guard_installed = True
        return True


__all__ = ["install_twitch_runtime_guard"]
