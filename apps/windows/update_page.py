...
def _set_notes(app: Any, text: str) -> None:
    update_ui._set_notes(app, text)  # noqa: SLF001
...
def test_proxy_settings(app: Any) -> None:
    ...
    def worker() -> None:
        try:
            ...
            text = f"检测成功：{result['mode']}，HTTP {result['status']}，延迟 {result['latency_ms']} ms。"
...
def build_update_tab(app: Any, frame: ttk.Frame, app_name: str, current_version: str, app_dir: Path) -> None:
    ...
    ttk.Label(inner, text=f"{app_name} 软件更新", font=("Microsoft YaHei UI", 16, "bold") if sys.platform == "win32" else ("", 16, "bold")).grid(row=0, column=0, sticky="w", pady=(4, 3))
    ...
    ttk.Label(version_row, text="版本").grid(row=0, column=2, sticky="w", padx=(0, 8))
    app._update_version_combo = ttk.Combobox(version_row, textvariable=app.update_version_var, state="disabled", width=42)
    app._update_version_combo.grid(row=0, column=3, sticky="ew")
    app._update_version_combo.bind("<<ComboboxSelected>>", lambda _event: update_version_selector.on_version_selected(app))

    app._update_progress = ttk.Progressbar(update_frame, mode="determinate", maximum=100, variable=app.update_progress_var)
    ...
    app._update_check_button = ttk.Button(button_row, text="检查更新", command=lambda: update_version_selector.check_for_versions(app, silent=False))
    app._update_check_button.pack(side="left", padx=(0, 8))
    app._update_full_button = ttk.Button(button_row, text="全量更新", command=lambda: update_version_selector.install_selected_full(app), state="disabled")
    ...
    ttk.Checkbutton(network, text="使用第三方代理", variable=app.update_third_party_proxy_var, command=lambda: _toggle_third_party(app)).grid(row=2, column=0, columnspan=2, sticky="w", pady=4)
    ...
    ttk.Button(action_row, text="保存网络设置", command=lambda: save_network_settings(app)).pack(side="left", padx=(0, 8))
    app._update_proxy_test_button = ttk.Button(action_row, text="检测连接", command=lambda: test_proxy_settings(app))
    app._update_proxy_test_button.pack(side="left")
    ttk.Checkbutton(network, text="使用 Mirror酱更新", variable=app.update_mirrorchyan_var, command=lambda: _toggle_mirrorchyan(app)).grid(row=6, column=0, columnspan=2, sticky="w", pady=(10, 2))
    ...
    ttk.Button(network, text="保存 CDK", command=lambda: _save_mirrorchyan_cdk(app)).grid(row=7, column=2, sticky="w", padx=(8, 0), pady=4)
    ttk.Label(network, text="当前仅保存 CDK，Mirror酱更新暂未接入；后续接入后可直接使用已保存的 CDK。", wraplength=760).grid(row=8, column=0, columnspan=3, sticky="w", padx=(12, 0), pady=(0, 5))
    ttk.Label(network, textvariable=app.update_proxy_status_var, wraplength=760).grid(row=9, column=0, columnspan=3, sticky="w", pady=(7, 0))

    load_network_settings(app)
    update_version_selector.check_for_versions(app, silent=True)


__all__ = ["RELEASE_CHANNELS", "build_update_tab", "load_network_settings", "save_network_settings", "test_proxy_settings"]
